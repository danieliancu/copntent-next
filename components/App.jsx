...
const App = () => {
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          filterData(selectedSource, selectedCategory, result.data);
        } else {
    ...
    // Configurăm intervalul pentru refresh automat
    const interval = setInterval(() => {
      fetchAllData();
    }, 300000); // La fiecare 5 minute (300000 ms)

    return () => clearInterval(interval); // Curățare interval la demontare
  }, [selectedSource, selectedCategory]); // Dependințe: sursa și categoria selectată
  ...
  const handleCategoryFilter = (category) => {
    ...
    window.scrollTo({ top: 0, behavior: "smooth" }); // Derulează la începutul paginii
